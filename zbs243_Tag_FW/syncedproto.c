...
static uint8_t __xdata lastDataVer[8] = {0};
static uint8_t __xdata lastDataImageSlot = 0xFF;
...
static uint32_t getAddressForLastDataVer()
{
    return EEPROM_SETTINGS_AREA_START;
}
...
{
    lastDataImageSlot = findSlot(lastDataVer);
    if (lastDataImageSlot != 0xFF)
    {
        pr("last data ver is valid\n");
    }
    else
    {
        pr("last data ver is invalid\n");
    }
}
